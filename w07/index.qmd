...
| | Start | End | Topic |
|:- |:- |:- |:- |
| **Lecture** | 6:30pm | 7:20pm | [BST Iteration](#back-to-bsts) |
...
## LLs $\rightarrow$ BSTs: The Hard Part {.crunch-title .title-09 .crunch-ul .text-90}

* When we were working with LinkedLists, we could access **all items** by just "looping through" (follow `next`, `print()` at each step^[This `print()` step is going to become important! We'll say we've **"processed" a node** once we've `print()`ed its `content`])
...
## Two Ways to Traverse: IRL Version {.smaller .crunch-title .title-12 .crunch-ul .crunch-callout .crunch-li-5 .callout-fix}
...
::: {.callout-note icon="false" title="<i class='bi bi-info-circle pe-1'></i> Depth-First Search (DFS)" style="margin-bottom: 8px !important; font-size: 76% !important;"}
...
::: {.callout-note icon="false" title="<i class='bi bi-info-circle pe-1'></i> Breadth-First Search (BFS)" style="font-size: 76% !important;"}
...
* ...Let's try them out! I clicked "Random Article" and got [Eustache de Saint Pierre](https://en.wikipedia.org/wiki/Eustache_de_Saint_Pierre)
...
## The Boxes {.text-65 .title-10 .crunch-title .crunch-images .crunch-quarto-figure .crunch-table}
...
<tbody>
<tr>
  <td class='tdvc'><span data-qmd="**Data Structures**"></span></td>
...
# Homework 3 {data-stack-name="HW3"}
...
## `HashTable`s: More Memory, More Performance
