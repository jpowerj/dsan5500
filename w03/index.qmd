...
* [**Key**]{.cb2} moves from left-to-right, once per "outer loop" iteration (a)-(e)
...
## The Figure You Can Make In Your Head All Semester {.smaller .crunch-title .title-09 .crunch-quarto-figure}
...
## Constants On Highest-Degree Terms Also Go Away {.smaller .crunch-title .title-09}
...
## Intuition $\rightarrow$ Proof {.crunch-title .crunch-ul .text-90}
...
* Given **any value $n > n_0$**, we need to **construct an input** for which Insertion-Sort requires $cn^2$ steps: consider a list where [$n/3$ **greatest values** are in **first $n/3$ slots**]{.boxed}:
...
## Doing Better Than Insertion Sort {data-stack-name="Merge Sort" .crunch-title .title-07 .crunch-ul .crunch-li .crunch-quarto-layout-panel .crunch-quarto-figure .text-80}

* *Intuition* 🥳: Recall [**finding a word in a dictionary**](https://dsan.io/dict-lookup) app!
* How can [Merge-Sort]{.alg} work **that** much better?!?
...
# Object-Oriented Programming Overview {data-stack-name="OOP"}
...
## Complexity With Respect To A *Structure* {.smaller .crunch-title .title-12 .crunch-li-8}
...
## Appendix: The Full Master Theorem {.smaller .crunch-title .title-11 .crunch-ol .crunch-ul .crunch-math-15}
